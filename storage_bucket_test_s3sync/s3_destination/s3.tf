...
resource "yandex_storage_bucket" "test-s3sync-destination-anton-patsev" {
  ...
  bucket     = "test-s3sync-destination-anton-patsev"

}

output "access_key_sa_storage_admin_for_test_bucket" {
  description = "access_key sa-storage-admin for test-s3sync-destination-anton-patsev"
  value       = yandex_storage_bucket.test-s3sync-destination-anton-patsev.access_key
  sensitive   = true
}

output "secret_key_sa_storage_admin_for_test_bucket" {
  description = "secret_key sa-storage-admin for test-s3sync-destination-anton-patsev"
  value       = yandex_storage_bucket.test-s3sync-destination-anton-patsev.secret_key
  sensitive   = true
}
