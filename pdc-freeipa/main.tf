...
output "public_ip_freeipa" {
  description = "Public IP address for freeipa"
...
data "template_file" "inventory_yml" {
  ...
  vars = {
    ...
    public_ip_freeipa      = module.freeipa.external_ip[0]
    letsencrypt_domain     = var.letsencrypt_domain
    pswd_freeipa_ldap_sync = var.pswd_freeipa_ldap_sync
