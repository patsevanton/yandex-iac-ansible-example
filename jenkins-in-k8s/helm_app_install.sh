...
external_ip=$( kubectl get services nginx-ingress-ingress-nginx-controller --output jsonpath='{.status.loadBalancer.ingress[0].ip}' )
export external_ip
jenkins_url=jenkins.$external_ip.sslip.io
export jenkins_url
echo ""
echo "jenkins_url:"
echo "$jenkins_url"
